...
def main(cfg: DictConfig) -> Optional[float]:
    ...
    for it in tqdm(
        ...
        trajectories["EKF"][it] = ekf_pos_pose
        nll["EKF"].append(ekf.neg_log_likelihood(gt_pose))
        # PF filter
        ...
        axes_filters = plot_se2_filters(
            {
                "HEF": [harmonic_pos_pose, posteriori_hat.prob.real],
                "EKF": [ekf_pos_pose, ekf_pos_cov],
                "PF": [pf_pos_pose, pf.particles],
                "HistF": [hf_pos_pose, hf.prior.reshape(grid_size)],
                "GT": [gt_pose, None],
            },
            x,
            y,
        ...
        for ax_mean, ax_filter in zip(axes_means, axes_filters):
