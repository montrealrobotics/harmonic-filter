...
def main(cfg: DictConfig) -> Optional[float]:
    ...
    mus = [np.array([0.0, -0.15, 0]), np.array([0.0, 0.15, 0])]
    covs = [np.diag(cfg.filter.var_prior), np.diag(cfg.filter.var_prior)]
    ...
    pf = RangePFBimodal(priors=mus, priors_cov=covs, n_particles=np.prod(grid_size))
    simulator = SE2RangeSimulator(
        start=SE2Group.from_parameters(*mus[0]),
