...
def create_mp4(path: str, video_name: str, duration=100):
    ...
    mask = np.zeros(image_paths.shape[0], dtype=bool)
    ...
    mean_paths = image_paths[~mask]
    filter_paths = image_paths[mask]

    output_images = []

    it = 0
    for m_path, f_path in zip(mean_paths, filter_paths):
        # Open the two input images
        image1 = Image.open(m_path)
        image2 = Image.open(f_path)
